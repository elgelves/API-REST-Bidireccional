...
public class BibliotecaController {
    ...
    @GetMapping
    public ResponseEntity<Page<Biblioteca>> listarBibliotecas(Pageable pegeable){
        return ResponseEntity.ok(bibliotecaRepository.findAll(pegeable));
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Biblioteca> obtenerBibliotecaxId(@PathVariable Integer id) {
        Optional<Biblioteca> bibliotecaOptional = bibliotecaRepository.findById(id);
        if (!bibliotecaOptional.isPresent()) {
            return ResponseEntity.unprocessableEntity().build();
        }
